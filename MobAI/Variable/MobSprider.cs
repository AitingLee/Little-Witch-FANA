...
public class MobSprider : MonoBehaviour
{
    ...
    private void SkillAttack()
    {
        ...
        if (distance < m_fProjectileRange)
        {
            ...
            skillFX = ObjectPoolManager.instance.GetWebProjectile();
            skillFX.transform.position = firePoint.transform.position;
            Vector3 direction = m_mobAI.playerM.transform.position - transform.position;
            direction.y = transform.forward.y;
            skillFX.transform.forward = direction;
            WebProjectile WP = skillFX.GetComponent<WebProjectile>();
            WP.attackPower = m_mobAI.m_fAttackPower;
            WP.webProjectileRate = m_fProjectileRate;
            skillFX.SetActive(true);

            skillFX.transform.SetParent(transform.parent);
            skillFX.SetActive(true);
