...
public class MobFledgling : MonoBehaviour
{
    ...
    [Header("Attack Rate")]
    public float m_fSkillAttackRate;

    [Header("Attack Range")]
    public float m_fNormalAttackRange;
    public float m_fSkillAttackRange;
    ...
    private void PerformAttack()
    {
        switch (m_fCurrentAttackNo)
        {
            ...
            case 2:
                //Projectile §Þ¯à
                if (m_mobAI.CheckPlayerInAttackRange(m_fSkillAttackRange))
                {
                    m_mobAI.AnimatorSetFloat("moveAmount", 0);
                    m_mobAI.AnimatorSetBool("isAttacking", true);
    ...
    private void NormalAttack()
    {
        ...
        if (angle < 20f && angle > -20f && distance < m_fNormalAttackRange + 1f)
        {
            ...
            AudioManager.instance.chickAttack.Play();
        }
        else
        {
    ...
    private void SkillAttack()
    {
        ...
        Vector3 playerDir = playerPlanePos - thisPlanePos;
        float distance = playerDir.magnitude;
        if (distance < m_fSkillAttackRange + 2f)
        {
            ...
            skillFX.Clear();
            skillFX.Play();
            m_mobAI.playerM.TakeDamage(Mathf.RoundToInt(m_mobAI.m_fAttackPower * m_fSkillAttackRate));
